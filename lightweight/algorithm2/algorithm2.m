function [centers,sampNum,z_prime,time] = algorithm2(data,k,z,varargin)
...
% eta, delta, ksi, e1, e2, k_prime are the parameters ¦Ç, ¦Ä, ¦Î, ¦Å1, ¦Å2, k'
tic
n_data = size(data,1);
p = inputParser;
% set defaults
...
defaultz_prime = 100; defaultTimes = 10; defaultValdnRatio = 0.1;
...
p.addParameter('z_prime',defaultz_prime,...
...
if sum(ismember(std,p.Parameters)) == 5
    ...
    z_prime = round(1/eta*e2/k*sampNum);
end
if ismember(sampNumGiven,p.Parameters) == 1
    sampNum = p.Results.sampNum;
end
if ismember(zpGiven,p.Parameters) == 1
    z_prime = p.Results.z_prime;
end
...
for i = 1:Times
    ...
    [~,centers_t] = streaming(S,k,z_prime);
    %
    if Times>1
...
% Sum_dis = Sum_sqdist(centers,data,z_prime);
end